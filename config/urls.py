...
urlpatterns += [
    url(r'^bs/', include(('competition.urls','bs'), namespace='bs')),  # 比赛
    url(r'^api/', include(('api.urls', 'api'),namespace='api')),  # 接口
    url(r'^biz/', include(('business.urls','biz'), namespace='biz')),  # 机构
    url(r'^ana/', include(('analysis.urls','ana'), namespace='ana')),  # 分析
]

urlpatterns += [
    url(r'^auth/', include(('account.urls','auth'), namespace='auth')),  # 微信授权
]
